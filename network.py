...
class Network(object):
    def __init__(self, obs, act_head_list, name, trainable=True, hidden_sizes=(128, 64), activation=tf.nn.relu, output_activation=None):
        ...
        with tf.variable_scope(self.name):
            ...
            for i, act_dim in enumerate(self.act_head_list):
                with tf.variable_scope('policy_net_head_{}'.format(i)):
                    ...
                    tmp_logits = tf.layers.dense(inputs=middle_out, units=act_dim, activation=None, trainable=trainable)
                self.final_out['policy_net_head_{}'.format(i)] = tmp_logits
        
            with tf.variable_scope('value_net'):
                self.value = tf.squeeze(self.mlp(middle_out, list(hidden_sizes) + [1], activation, output_activation, trainable=trainable), axis=-1)
    ...
    def output(self):
        head_dist_dict = {'policy_net_head_{}'.format(i): Categorical(logits=logits) for i, logits in enumerate(self.final_out.values())}
        params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.name)

        return head_dist_dict, self.value, params
